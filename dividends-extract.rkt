...
(define (download-dividends symbols)
  ...
  (call-with-output-file (string-append "/var/tmp/iex/dividends/" (date->string (current-date) "~1") "/"
                                        (first symbols) "-" (last symbols) ".json")
    (λ (out)
      (~> (string-append "https://cloud.iexapis.com/beta/stock/market/batch?symbols=" (string-join symbols ",")
                         "&types=dividends&range=" (history-range) "&token=" (api-token))
          (string->url _)
          (get-pure-port _)
          (copy-port _ out)))
    #:exists 'replace))
...
(with-task-server (for-each (λ (l) (schedule-delayed-task (λ () (download-dividends (first l)))
                                                          (second l)))
